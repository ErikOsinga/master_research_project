...
theta1_fid = 1. # mean of 1st dimension (x)
theta2_fid = 2. # mean of 2nd dimension (y)
delta_theta = 0.1
...
def plot_data():
	...
	plt.savefig('./Figures/multivariate_gaussian/data_visualization_mean.png')
	plt.show()
	plt.close()
...
def plot_derivatives():
	...
	plt.savefig('./Figures/multivariate_gaussian/derivatives_mean.png')
	plt.show()
	plt.close()
...
def plot_variables():
	...
	plt.savefig('./Figures/multivariate_gaussian/variables_vs_epochs_mean.png')
	plt.show()
...
def ABC():
	...
	theta, summary, s, ro, F = n.ABC(real_data = real_data, prior = [0, 4]
	...
	epsilon = 1e10 # Chosen quite arbitrarily
	...
	def plot_samples():
		...
		plt.savefig('./Figures/multivariate_gaussian/approximate_bayesian_computation_mean.png')
		plt.show()
...
def PMC_ABC():
	...
	def plot():
		...
		ax[1, 1].set_ylabel('$\\mathcal{P}(\\theta|{\\bf d})$')
		ax[1, 1].set_yticks([]);
		plt.savefig('./Figures/multivariate_gaussian/PMC_ABC_mean.png')
		plt.show()
	plot()

	return theta_

np.pmc_not_implemented()
theta_ =  PMC_ABC()
...
ax.set_ylabel('$\\mathcal{P}(\\theta|{\\bf d})$')
ax.set_yticks([])
# plt.savefig('./Figures/likelihoods_mean.png')
plt.show()