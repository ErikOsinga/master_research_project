...
def generate_data(theta, train=False):
	...
	'''
	if train:
		return np.moveaxis(np.random.normal(theta, 1.
			, [1] + input_shape + [len(theta)]), -1, 0)
	else: 
		return np.moveaxis(np.random.normal(theta, 1.
			, input_shape + [len(theta)]), -1, 0)

# 10 data points of Gaussian ?? mean 0.25 std noise
input_shape = [100]
# Fiducial parameter and perturbed values just above and below
theta_fid = 5. # The mean
theta_real = theta_fid
delta_theta = 0.1
...
def plot_data():
	...
	plt.savefig('./Figures/1d_gaussian_mean/data_visualizationvar=1_100data.png')
	plt.close()
...
def plot_derivatives():
	...
	plt.savefig('./Figures/1d_gaussian_mean/derivatives_visualizationvar=1_100data.png')
	plt.close()
...
def plot_variables():
	...
	plt.savefig('./Figures/1d_gaussian_mean/variables_vs_epochsvar=1_100data.png')
	plt.close()
	# plt.show()
...
real_data = generate_data([theta_real], train = False)
...
def ABC():
	...
	def plot_samples():
		...
		ax[1].set_yticks([])
		plt.savefig('./Figures/1d_gaussian_mean/ABCvar=1_100data.png')
		plt.close()
...
def PMC_ABC():
	...
	def plot():
		...
		ax[1].set_ylabel('$\\mathcal{P}(\\theta|{\\bf d})$')
		ax[1].set_yticks([]);
		plt.savefig('./Figures/1d_gaussian_mean/PMCABCvar=1_100data.png')
		plt.close()
	plot()
...
ax.set_ylabel('$\\mathcal{P}(\\theta|{\\bf d})$')
ax.set_yticks([])
plt.savefig('./Figures/likelihoods_seed123_2var=1_100data.png')
# plt.show()