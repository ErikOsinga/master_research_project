...
def generate_data(theta1, n_samples = 1, train=False):
	'''
...
theta1_fid = 1.0 # variance of 1st dimension (x)
delta_theta = 0.1
...
t = generate_data(theta1_fid,n_s*n_train, train = False)
...
t_m1 = generate_data(theta1_fid - delta_theta,n_train*n_p, train = True) 
t_m = t_m1
np.random.seed(seed)
t_p1 = generate_data(theta1_fid + delta_theta,n_train*n_p, train = True) 
t_p = t_p1
...
''' Generate test data '''
tt = generate_data(theta1_fid, n_s, train=False)
seed = np.random.randint(1e6)
np.random.seed(seed)
tt_m1 = generate_data(theta1_fid - delta_theta, n_p, train=True)
tt_m = tt_m1
np.random.seed(seed)
tt_p1 = generate_data(theta1_fid + delta_theta, n_p, train=True)
tt_p = tt_p1
np.random.seed()
...
def plot_data():
	...
	# plt.savefig('./multivariate_gaussian_one_unkown_variance/data_visualization')
	# plt.show()
	...
	plt.savefig('./Figures/multivariate_gaussian_one_unkown_variance/data_visualization_theta=%.2f.png'%theta1_fid)
	# plt.show()
	plt.close()
...
def plot_derivatives():
	...
	ax[0, 0].set_ylim(0-3*theta1_fid,0+3*theta1_fid)
	...
	ax[1, 0].set_ylim(0-3*theta1_fid,0+3*theta1_fid)
	...
	plt.savefig('./Figures/multivariate_gaussian_one_unkown_variance/derivatives_theta=%.2f.png'%theta1_fid)
	# plt.show()
	plt.close()
...
parameters = {
    ...
    'fiducial θ': np.array([theta1_fid]),
...
def plot_variables():
	...
	ax[3].plot(epochs, np.array(n.history["dμdθ"])[:,0].reshape((np.prod(np.array(n.history["dμdθ"]).shape)))
	...
	ax[3].plot(epochs, np.array(n.history["test dμdθ"])[:,0].reshape((np.prod(np.array(n.history["test dμdθ"]).shape)))
	...
	plt.savefig('./Figures/multivariate_gaussian_one_unkown_variance/variables_vs_epochs_theta=%.2f.png'%theta1_fid)
	# plt.show()
	plt.close()
...
real_data = generate_data(theta1_fid, 1, train = False)
...
def ABC():
	...
	epsilon = abs(summary[0]/10) # Chosen quite arbitrarily
	...
	def plot_samples():
		...
		plt.subplots_adjust(hspace = 0)
		theta1 = theta # only 1 theta
		# theta2 = theta[:,1]

		ax[0].set_title('Epsilon is chosen to be %.2f'%epsilon)
		ax[0].scatter(theta1[accept_indices] , s[accept_indices, 0], s = 1)
		ax[0].scatter(theta1[reject_indices], s[reject_indices, 0], s = 1, alpha = 0.1)
		ax[0].plot([0, 10], [summary[0], summary[0]], color = 'black', linestyle = 'dashed')
		ax[0].set_ylabel('Network output', labelpad = 0)
		ax[0].set_xlim([0, 10])
		ax[1].hist(theta1[accept_indices], np.linspace(0, 10, 100)
			, histtype = u'step', density = True, linewidth = 1.5, color = '#9467bd');
		ax[1].set_xlabel('$\\theta_1$')
		ax[1].set_ylabel('$\\mathcal{P}(\\theta|{\\bf d})$')
		ax[1].set_yticks([])

		plt.savefig('./Figures/multivariate_gaussian_one_unkown_variance/approximate_bayesian_computation_theta=%.2f.png'%theta1_fid)
		plt.show()
...
def PMC_ABC():
	...
	theta_, summary_, ro_, s_, W, total_draws, F = n.PMC(real_data = real_data
		, prior = [0, 5], num_draws = 10000, num_keep = 10000
	...
	def plot():
		...
		ax[1, 1].set_yticks([]);
		plt.savefig('./Figures/multivariate_gaussian_one_unkown_variance/PMC_ABC_theta=%.2f.png'%theta1_fid)
		plt.show()
	plot()
...
ax.set_ylabel('$\\mathcal{P}(\\theta|{\\bf d})$')
ax.set_yticks([])
# plt.savefig('./Figures/likelihoods_theta=%.2f.png'%theta1_fid)
plt.show()