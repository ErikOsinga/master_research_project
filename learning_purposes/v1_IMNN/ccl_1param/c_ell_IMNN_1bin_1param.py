...
class nholder(object):
    ...
    def __init__(self, input_shape, generate_data, theta_fid, delta_theta, n_s, n_train, 
        ...
        version, fromdisk, noiseless_deriv, flatten):
        """
        ...
        if fromdisk:
            self.data, self.der_den = self.load_data_from_disk()
        else:
    ...
    def create_network(self):
        ...
        # Make sure that the latest version of the data is passed to the network
        self.parameters['preload data'] = self.data
    ...
    def rescale_data(self):
        ...
        std = np.nanstd(self.data['x_central'])
        mean = np.nanmean(self.data['x_central'])
        for key in self.data.keys():
...
# IMNN PARAMETERS 
#########################################
# The input shape is a 1D vector of length 1*len(ells)
...
num_epochs = int(10e3)
keep_rate = 0.9 # 1 minus the dropout
verbose = 0
...
noiseless_deriv = False # whether to add noise to upper/lower simulations
...
nholder1 = nholder(input_shape, generate_data, theta_fid, delta_theta, n_s,
        ...
        verbose, version, fromdisk, noiseless_deriv, flatten)
...
# HAVE TO UNCOMMENT IN IMNN.PY FOR THIS TO WORK
...
# # Perform ABC
prior = [[0.211-0.11, 0.211+0.11]] # [ [prior1], [prior2], etc.. ]
draws = 1000
