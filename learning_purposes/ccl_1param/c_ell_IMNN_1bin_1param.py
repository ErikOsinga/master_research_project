...
def generate_data(θ, train=None, flatten=False):
    ...
    def helper_func(θ):
        ...
        if (θ[:,0] == θ[0,0]).all():
            ...
            else: # if it is for calculating derivatives, just use the noise free version
        ...
        # Omega_c, sigma8 = θ,  not possible if they are different params
            ...
            all_Cls = []
            for Omega_c in θ: # can be done in parallel for all i 

                Cls, dNdzs = euclid_ccl(Omega_c[0])
