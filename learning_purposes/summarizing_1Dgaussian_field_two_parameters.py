...
t = generate_data([theta_fid+theta2_fid for i in range(n_train * n_s)], train = False)

# Hypothesis: maximum likelihood should combine to 3, but independent thetas can
# be very different, might be too hard for the ABC or PMC to learn..?
...
np.random.seed(seed)
t_m1 = generate_data([theta_fid - delta_theta + theta2_fid 
	for i in range(n_train*n_p) ], train = True) 
# these are both the same simulation actually but yeah..
t_m2 = generate_data([theta_fid + theta2_fid - delta_theta
						for i in range(n_train*n_p)], train= True)
t_m = np.concatenate( (t_m1, t_m2), axis=1) # I think this is the correct way
np.random.seed(seed)
t_p1 = generate_data([theta_fid + delta_theta + theta2_fid
						for i in range(n_train*n_p)], train = True) 
# these are both the same simulation actually but yeah..
t_p2 = generate_data([theta_fid + theta2_fid + delta_theta
						for i in range(n_train*n_p)], train= True)
...
tt = generate_data([theta_fid+theta2_fid for i in range(n_s)], train=False)
seed = np.random.randint(1e6)
np.random.seed(seed)
tt_m1 = generate_data([theta_fid - delta_theta + theta2_fid
	for i in range(n_p)], train=True)
tt_m2 = generate_data([theta_fid + theta2_fid - delta_theta
	for i in range(n_p)], train=True)
tt_m = np.concatenate( (tt_m1, tt_m2), axis=1)
np.random.seed(seed)
tt_p1 = generate_data([theta_fid + delta_theta + theta2_fid
	for i in range(n_p)], train=True)
tt_p2 = generate_data([theta_fid + theta2_fid + delta_theta
	for i in range(n_p)], train=True)
...
def plot_data():
	...
	plt.savefig('./Figures/1d_gaussian_field2params/data_visualization_dense.png')
	plt.close()
...
def plot_derivatives():
	...
	plt.savefig('./Figures/1d_gaussian_field2params/derivatives_visualization_dense.png')
	plt.close()
...
def plot_variables():
	...
	plt.savefig('./Figures/1d_gaussian_field2params/variables_vs_epochs_dense.png')
	plt.close()
	# plt.show()
...
real_data = generate_data([3.], train = False)
...
def ABC():
	...
	def show_real_data():
		...
		plt.savefig('./Figures/1d_gaussian_field2params/real_data_dense.png')
		plt.close()
	...
	ϵ = 200 # chosen quite arbitrarily
	...
	def plot_samples():
		...
		ax[0].set_xlim([0, 10])
		ax[1].hist(theta[accept_indices], np.linspace(0, 10, 100), histtype = u'step', density = True, linewidth = 1.5, color = '#9467bd');
		ax[1].set_xlabel('$\\theta$')
		ax[1].set_ylabel('$\\mathcal{P}(\\theta|{\\bf d})$')
		ax[1].set_yticks([])
		plt.savefig('./Figures/1d_gaussian_field2params/ABC.png')
		plt.close()
...
def PMC_ABC():
	...
	def plot():
		...
		ax[1].set_ylabel('$\\mathcal{P}(\\theta|{\\bf d})$')
		ax[1].set_yticks([]);
		plt.savefig('./Figures/1d_gaussian_field2params/PMC_ABC.png')
		plt.close()
		# plt.show()
	plot()
...
def first_order_Gaussian_MLE():
	...
	def plot():
		...
		ax.set_yticks([])
		plt.savefig('./Figures/1d_gaussian_field2params/Gaussian_MLE_dense.png')
		plt.close()
...
ax.set_ylabel('$\\mathcal{P}(\\theta|{\\bf d})$')
ax.set_yticks([])
plt.savefig('./Figures/1d_gaussian_field2params/likelihoods_dense.png')
# plt.show()
plt.close()